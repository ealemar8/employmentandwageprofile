...
# wages
wage = aggregate(masterdat$avg_annual_pay ~ masterdat$year + masterdat$industry_title + masterdat$area_title, FUN = mean)
names(wage) = c('Year', 'Industry', 'Municipality', 'Wages')
all_munis = aggregate(masterdat$avg_annual_pay ~ masterdat$year +
                        masterdat$industry_title, FUN = mean)
all_industries = aggregate(masterdat$avg_annual_pay ~masterdat$year +
                             masterdat$area_title, FUN = mean)
all = aggregate(masterdat$avg_annual_pay ~ masterdat$year, FUN = mean)
wage = rbind(wage, data.frame("Year"=all_munis$`masterdat$year`, "Industry"=all_munis$`masterdat$industry_title`, "Municipality"=paste0('All Municipalities'), 'Wages'=all_munis$`masterdat$avg_annual_pay`))
wage = rbind(wage, data.frame("Year"=all_industries$`masterdat$year`, "Industry"=paste0('All Industries'), "Municipality"=all_industries$`masterdat$area_title`, "Wages"=all_industries$`masterdat$avg_annual_pay`))
wage = rbind(wage, data.frame('Year'=all$`masterdat$year`,
                                'Industry'=paste0('All Industries'), 'Municipality'=paste0('All Municipalities'), 'Wages'=all$`masterdat$avg_annual_pay`))
```
...
renderPlot({
  ggplot(data = wage[wage$Municipality==input$muni&wage$Industry==input$industry, ], 
         aes(x = Year, y = Wages)) +
    geom_line() +
    geom_text(data = wage[wage$Municipality==input$muni&wage$Industry==input$industry, ], aes(y = Wages, label = dollar(Wages))) + ggtitle(paste0('Average Annual Wage in ', input$muni, ' - ', input$industry))  
})
```
